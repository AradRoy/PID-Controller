...
static int index=0, polyOrder;
static double xArrDouble[10], yArrDouble[10], yPolyData[100], polynom[5], polyError;
	...
	while(GetInQLen (portVal) > 0) //if the input is not empty
	{
		...
		if(bytesRead>0)
		{
			...
			/*
			  ...
			  #
				...
				____reciving___
			  # if the first character is a NUMBER means that its data to graph
				goes in to array.	format: (int , int)
			
			  # if the first character is a LETTER means its text to display
				format ( text )
			
			  # if the end character is * means its data from case 2
			
			  # if the end character is $ means its data from case 3
        ...
        //_____________________CASE 3_____________________________
			if(message[0]==36)		// ASCII for '$'
			{
				...
				GetCtrlVal (panelHandle, PANEL_NUMERIC_3, &polyOrder);
				PolyFit (xArrDouble, yArrDouble, 10, polyOrder, yPolyData, polynom, &polyError);
				
				double xPolyCheck[100],yPolyCheck[100];
				
				Ramp (100, xArrDouble[0], xArrDouble[9], xPolyCheck);
				
				PolyEv1D (xPolyCheck, 100, polynom, polyOrder+1, yPolyCheck);
...
int CVICALLBACK Connect (int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			SetCtrlAttribute (panelHandle, PANEL_COMMANDBUTTON_6, ATTR_DIMMED, 0);
...
int CVICALLBACK exportToPng (int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			int graphBitmap;
			char dataToGraph[100];//, imageName[100];
...
int CVICALLBACK runCalibrationTest (int panel, int control, int event,
									void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			double target[1], duty[1];
			int dutyCast;
			char message[20];
			
			GetCtrlVal (panelHandle, PANEL_NUMERIC_2, target);
			PolyEv1D (target, 1, polynom, polyOrder+1, duty);
			dutyCast=(int)duty[0];
			sprintf (message, "2,%d", dutyCast);
			
			SetCtrlVal(panelHandle, PANEL_STRING, message);
